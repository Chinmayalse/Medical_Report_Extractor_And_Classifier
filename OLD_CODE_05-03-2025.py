...
def classify_report(corrected_text, system_prompt_content):
    ...
    try:  # First attempt with Gemini
        ...
        if (  # Check for Gemini failure
            ...
            print("Gemini classification failed or unknown, trying Azure...")
...
def verify_results_with_gemini(enhanced_text, image_paths, report_type):
    ...
    This function serves as both a fallback and parallel verification method:
    1. Parses and validates the enhanced text
    2. Uses Gemini's multimodal capabilities for image analysis
    3. Maintains consistent data structure
    4. Provides additional validation layer
    
    Args:
        enhanced_text (str): Text to verify, either raw or JSON-formatted
        image_paths (list): Paths to report images for visual verification
        report_type (str): Type of medical report for context
...
def process_pdf(pdf_path):
    ...
    classification_result = classify_report(combined_text, classification_system_prompt)
    ...
    # Add classification result to the verification result
    if isinstance(verification_result, str):
        try:
            ...
            verification_result = json.dumps(verification_json)
        except json.JSONDecodeError:
            pass
